...
# def apply_ffmpeg_command(directory_path, gpu_vendor):
...
#                 command = f'ffmpeg -i "{input_file}" -c:v {codec} -b:v 1500k -c:a copy "{output_file}"'
...
# apply_ffmpeg_command(directory_path, gpu_vendor)
# -------------------------------
# import os
# import subprocess
# import sys
...
# def apply_ffmpeg_command(directory_path, gpu_vendor, codec):
#     total_start_time = time.time()  # Время начала выполнения скрипта
...
#             if file.endswith('.mp4'):
#                 input_file = os.path.join(root, file)
#                 output_file = os.path.join(root, '.'.join(file.split('.')[:-1]) + f'_{codec}_mod.mp4')
#                 command = f'ffmpeg -i "{input_file}" -c:v {codec_name} -b:v 1500k -c:a copy "{output_file}"'
...
#                 os.remove(input_file)  # Удаление исходного файла после конвертации
#                 # Переименование выходного файла без суффикса "_mod"
#                 new_output_file = os.path.join(root, '.'.join(file.split('.')[:-1]) + f'_{codec}.mp4')
#                 os.rename(output_file, new_output_file)
...
# directory_path = input("Введите путь к директории: ")
# gpu_vendor = input("Введите производителя видеокарты (n - NVIDIA, a - AMD): ").lower()
# codec = input("Введите кодек (h264 или h265): ")
# apply_ffmpeg_command(directory_path, gpu_vendor, codec)
