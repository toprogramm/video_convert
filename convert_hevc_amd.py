...
#                 command = f'ffmpeg -i "{input_file}" -c:v hevc_amf -b:v 1500k -c:a copy "{output_file}"'
#                 os.system(command)
...
#                 print(f"{Fore.RED}Исходный файл: {input_file}, Размер: {int(input_size / 1048576)} мбайт{Style.RESET_ALL}")
#                 print(f"{Fore.RED}Конечный файл: {output_file}, Размер: {int(output_size / 1048576)} мбайт{Style.RESET_ALL}")
#                 print(f"{Fore.GREEN}Время кодирования файла: {end_time - start_time:.2f} секунд{Style.RESET_ALL}")
...
#     print(f"{Fore.BLUE}Общее время выполнения скрипта: {total_end_time - total_start_time:.2f} секунд{Style.RESET_ALL}")
